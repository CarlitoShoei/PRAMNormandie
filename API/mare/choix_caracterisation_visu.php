<?php
	...
	$Session = $_GET['id_structure_conectee'];
...
<?php if($count_observation >= 1){ ?>
		<?php if($role == "administrateur"){ ?>
					<div id="textafficahge">	
						...
						<h3>Caractérisation de la mare : <?php echo $L_ID ?></h3>
							<div id="resultat">
								<table border="0" align="left" width="100%" height="20%">
									<tr>
										<td width="25%" align="center" >
											<p>Pour accéder à une caractérisation de la mare, merci de sélectionner dans la liste déroulante la date de caractérisation que vous souhiatez visionner :</p>
										</td>
										<td width="20%">
											<select style="width:100%" id="ID_CARAC" onchange="load_page('mare/description.php?L_ID=<?php echo $L_ID ?>&Session=<?php echo $Session ?>&id_structure_conectee=<?php echo $id_structure_conectee ?>&role=<?php echo $role ?>', 'affichage', 'affichageCaracMare')">
		...
		<?php }elseif($role == "observateur"){ ?>
			<?php if($count_contour >= 1){?>
						<?php if(pg_num_rows($observation_obs_ac_contour) == 0){ ?>
							...
							<h3>Caractérisation de la mare : <?php echo $L_ID ?></h3>
								<div id="resultat">			
									<p>Une caractérisation de la mare est disponibles, cependant les droits qui vous ont été affectés nous vous permettent pas d'accéder aux données. 
