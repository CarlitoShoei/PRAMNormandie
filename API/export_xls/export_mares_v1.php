...
elseif($role_structure['ROLE'] == "observateur"){
	...
	$req_filtre = pg_query($bdd, 'SELECT "ID", "L_ID", "L_IDSTRP", "L_STRP", "L_NOM", "STATUT", "L_ADMIN", "L_DATE", "L_OBSV", "L_PROP", "L_COOX", "L_COOY", "L_COOX93", "L_COOY93", "S_ID", "STRUCTURE", "Nom_Commune", "OBS_PRENOM", "OBS_NOM", "PROPR", "L_VALID", "OBS_NOM_PRENOM", "C_COMT" 
									FROM (SELECT saisie_observation.localisation."ID" AS "ID", saisie_observation.localisation."L_ID", saisie_observation.localisation."L_IDSTRP", "L_STRP", "L_NOM", "STATUT", "L_ADMIN", "L_DATE", "L_OBSV", "L_PROP", "L_COOX", "L_COOY", "L_COOX93", "L_COOY93", "S_ID", "STRUCTURE", "Nom_Commune", "OBS_PRENOM", "OBS_NOM", "PROPR", "L_VALID", "OBS_NOM_PRENOM", saisie_observation.localisation."C_COMT"
											...
											AND caracterisation."C_DATE" > '."'946681200'".' AND caracterisation."C_DATE" is not Null
...
elseif($role_structure['ROLE'] == "utilisateur"){
	...
	$req_filtre = pg_query($bdd, 'SELECT "ID", "L_ID", "L_IDSTRP", "L_STRP", "L_NOM", "STATUT", "L_ADMIN", "L_DATE", "L_OBSV", "L_PROP", "L_COOX", "L_COOY", "L_COOX93", "L_COOY93", "S_ID", "STRUCTURE", "Nom_Commune", "OBS_PRENOM", "OBS_NOM", "PROPR", "L_VALID", "OBS_NOM_PRENOM", "C_COMT" 
									FROM (SELECT saisie_observation.localisation."ID" AS "ID", saisie_observation.localisation."L_ID", saisie_observation.localisation."L_IDSTRP", "L_STRP", "L_NOM", "STATUT", "L_ADMIN", "L_DATE", "L_OBSV", "L_PROP", "L_COOX", "L_COOY", "L_COOX93", "L_COOY93", "S_ID", "STRUCTURE", "Nom_Commune", "OBS_PRENOM", "OBS_NOM", "PROPR", "L_VALID", "OBS_NOM_PRENOM", saisie_observation.localisation."C_COMT"
											...
											AND localisation."L_DATE" > '."'946681200'".' AND localisation."L_DATE" is not Null
											AND saisie_observation.structure."S_ID_SESSION" = '."'".$Identifiant_Session."'".'
											UNION ALL
											SELECT saisie_observation.localisation."ID" AS "ID", saisie_observation.localisation."L_ID", saisie_observation.localisation."L_IDSTRP", "L_STRP", "L_NOM", "STATUT", "L_ADMIN", "L_DATE", "L_OBSV", "L_PROP", "L_COOX", "L_COOY", "L_COOX93", "L_COOY93", "S_ID", "STRUCTURE", "Nom_Commune", "OBS_PRENOM", "OBS_NOM", "PROPR", "L_VALID", "OBS_NOM_PRENOM", saisie_observation.localisation."C_COMT"
											FROM saisie_observation.localisation, menu_deroulant.l_statut, saisie_observation.structure, ign_bd_topo.commune, saisie_observation.observateur, menu_deroulant.l_propr
...
while($resultat = pg_fetch_array($req_filtre)){
	...
	$worksheet->write("B".$x,utf8_decode($resultat['L_IDSTRP']));
	$worksheet->write("C".$x,utf8_decode($resultat['L_NOM']));
	...
	$worksheet->write("J".$x,utf8_decode($resultat['STATUT']));
	$worksheet->write("K".$x,date('d/m/Y', $resultat['L_DATE']));
	$worksheet->write("L".$x,utf8_decode($resultat['OBS_NOM_PRENOM']));
	$worksheet->write("M".$x,utf8_decode($resultat['STRUCTURE']));
	$worksheet->write("N".$x,utf8_decode($resultat['PROPR']));
	$worksheet->write("O".$x,utf8_decode($resultat['L_COMT']));
	$worksheet->write("P".$x,utf8_decode($resultat['L_VALID']));
		...
		if($role_structure['ROLE'] == "administrateur"){
			$req_filtre_carac = pg_query($bdd, 'SELECT saisie_observation.caracterisation."ID_CARAC" AS "ID_CARAC", "C_STRP", saisie_observation.caracterisation."L_ID" AS "L_ID", "C_DATE", "OBS_NOM_PRENOM", (SELECT "STRUCTURE" FROM saisie_observation.structure WHERE structure."S_ID"::text = "C_STRP"::text), "TYPE", "VEGET", "EVOLUTION", 
													"ABREUV", "TOPO", "C_TOPO", "C_TOPO_AUTRE", "CLOTURE", "HAIE", "FORM", "C_LONG", "C_LARG", "PROF", "NATFOND", "C_NATFOND", "C_NATFOND_AUTRE", "BERGES", "BOURRELET", 
													...
													"C_RECOU_HYDROPHYTE_E", "C_RECOU_HYDROPHYTE_NE", "C_RECOU_ALGUE", "C_RECOU_EAU_LIBRE", "C_RECOU_NON_VEGET", "EMBROUS", "OMBRAGE", "PATRIMOINE", "C_PATRIMOINE", "C_PATRIMOINE_AUTRE", saisie_observation.caracterisation."C_COMT" AS "C_COMT"
										...
										LEFT JOIN menu_deroulant.c_patrimoine ON saisie_observation.caracterisation."C_PATRIMOINE" = menu_deroulant.c_patrimoine."ID"
		...
		elseif($role_structure['ROLE'] == "observateur"){
			...
			$req_filtre_carac = pg_query($bdd, 'SELECT saisie_observation.caracterisation."ID_CARAC" AS "ID_CARAC", "C_STRP", saisie_observation.caracterisation."L_ID" AS "L_ID", "C_DATE", "OBS_NOM_PRENOM", (SELECT "STRUCTURE" FROM saisie_observation.structure WHERE structure."S_ID"::text = "C_STRP"::text), "TYPE", "VEGET", "EVOLUTION", 
										...
										LEFT JOIN saisie_observation.observateur ON saisie_observation.caracterisation."C_OBSV" = saisie_observation.observateur."ID"
										LEFT JOIN saisie_observation.localisation ON saisie_observation.localisation."L_ID" = saisie_observation.caracterisation."L_ID"
										LEFT JOIN saisie_observation.structure ON st_intersects(structure.geom, st_transform(saisie_observation.localisation.geom,2154))
										LEFT JOIN menu_deroulant.c_abreuv ON saisie_observation.caracterisation."C_ABREUV" = menu_deroulant.c_abreuv."ID"
										LEFT JOIN menu_deroulant.c_berges ON saisie_observation.caracterisation."C_BERGES" = menu_deroulant.c_berges."ID"
										LEFT JOIN menu_deroulant.c_bourrelet ON saisie_observation.caracterisation."C_BOURRELET" = menu_deroulant.c_bourrelet."ID"
										LEFT JOIN menu_deroulant.c_cloture ON saisie_observation.caracterisation."C_CLOTURE" = menu_deroulant.c_cloture."ID"
										...
										LEFT JOIN menu_deroulant.c_type ON saisie_observation.caracterisation."C_TYPE" = menu_deroulant.c_type."ID"
										LEFT JOIN menu_deroulant.c_veget ON saisie_observation.caracterisation."C_VEGET" = menu_deroulant.c_veget."ID"
										WHERE saisie_observation.structure."S_ID_SESSION" = '."'".$Identifiant_Session."'".'
										AND caracterisation."L_ID" = '."'".$resultat['L_ID']."'".'
										AND caracterisation."C_DATE" > '."'946681200'".' AND caracterisation."C_DATE" is not Null
										ORDER BY saisie_observation.caracterisation."ID_CARAC"');
			}else{
			$req_filtre_carac = pg_query($bdd, 'SELECT saisie_observation.caracterisation."ID_CARAC" AS "ID_CARAC", "C_STRP", saisie_observation.caracterisation."L_ID" AS "L_ID", "C_DATE", "OBS_NOM_PRENOM", (SELECT "STRUCTURE" FROM saisie_observation.structure WHERE structure."S_ID"::text = "C_STRP"::text), "TYPE", "VEGET", "EVOLUTION", 
										...
										WHERE saisie_observation.structure."S_ID_SESSION" = '."'".$Identifiant_Session."'".'
										AND caracterisation."L_ID" = '."'".$resultat['L_ID']."'".'
										AND (saisie_observation.caracterisation."C_DATE" > '."'946681200'".' AND saisie_observation.caracterisation."C_DATE" is not Null)
										ORDER BY saisie_observation.caracterisation."ID_CARAC"');
			}
		}
		elseif($role_structure['ROLE'] == "utilisateur"){
		...
		while($resultat_carac = pg_fetch_array($req_filtre_carac)){
			...
			if($resultat_carac['C_COULEUR'] == "3"){
				...
				$C_COULEUR_PRECISION = "/"." \ ".$C_COULEUR_PRECISION;
			}
			...
			if($resultat_carac['C_PATRIMOINE'] == "6"){
				...
				$C_PATRIMOINE_AUTRE = "/"." \ ".$C_PATRIMOINE_AUTRE;	;
			}
			$C_COMT = $resultat_carac['C_COMT']." \ ".$C_COMT;
				...
				while($resultat_alim = pg_fetch_array($req_filtre_alim)){
					...
					if($resultat_alim['ALIMENTATION'] == "autre"){
						...
						$ALIMENTATION_AUTRE = "/"." \ ".$ALIMENTATION_AUTRE;	
					}
				}
		...
		$worksheet->write("Q".$x,$ID_CARAC); // ici on va ꤲire une cellules bien dꧩnie
		$worksheet->write("R".$x,$L_ID); // ici on va ꤲire une cellules bien dꧩnie
		$worksheet->write("S".$x,$C_DATE);
		$worksheet->write("T".$x,utf8_decode($OBS_NOM_PRENOM));
		$worksheet->write("U".$x,utf8_decode($STRUCTURE));
		$worksheet->write("V".$x,utf8_decode($TYPE));
		$worksheet->write("W".$x,utf8_decode($VEGET));
		$worksheet->write("X".$x,utf8_decode($EVOLUTION));
		$worksheet->write("Y".$x,utf8_decode($ABREUV));
		$worksheet->write("Z".$x,utf8_decode($TOPO));
		$worksheet->write("AA".$x,utf8_decode($C_TOPO_AUTRE));
		$worksheet->write("AB".$x,utf8_decode($CLOTURE));
		$worksheet->write("AC".$x,utf8_decode($HAIE));
		$worksheet->write("AD".$x,utf8_decode($FORM));
		$worksheet->write("AE".$x,utf8_decode($C_LONG));
		$worksheet->write("AF".$x,utf8_decode($C_LARG));
		$worksheet->write("AG".$x,utf8_decode($PROF));
		$worksheet->write("AH".$x,utf8_decode($NATFOND));
		$worksheet->write("AI".$x,utf8_decode($C_NATFOND_AUTRE));
		$worksheet->write("AJ".$x,utf8_decode($BERGES));
		$worksheet->write("AK".$x,utf8_decode($BOURRELET));
		$worksheet->write("AL".$x,utf8_decode($C_BOURRELET_POURCENTAGE));
		$worksheet->write("AM".$x,utf8_decode($PIETINEMENT));
		$worksheet->write("AN".$x,utf8_decode($HYDROLOGIE));
		$worksheet->write("AO".$x,utf8_decode($TURBIDITE));
		$worksheet->write("AP".$x,utf8_decode($COULEUR));
		$worksheet->write("AQ".$x,utf8_decode($C_COULEUR_PRECISION));
		$worksheet->write("AR".$x,utf8_decode($TAMPON));
		$worksheet->write("AS".$x,utf8_decode($EXUTOIRE));
		$worksheet->write("AT".$x,utf8_decode($C_RECOU_TOTAL));
		$worksheet->write("AU".$x,utf8_decode($C_RECOU_HELOPHYTE));
		$worksheet->write("AV".$x,utf8_decode($C_RECOU_HYDROPHYTE_E));
		$worksheet->write("AW".$x,utf8_decode($C_RECOU_HYDROPHYTE_NE));
		$worksheet->write("AX".$x,utf8_decode($C_RECOU_ALGUE));
		$worksheet->write("AY".$x,utf8_decode($C_RECOU_EAU_LIBRE));
		$worksheet->write("AZ".$x,utf8_decode($C_RECOU_NON_VEGET));
		$worksheet->write("BA".$x,utf8_decode($EMBROUS));
		$worksheet->write("BB".$x,utf8_decode($OMBRAGE));
		$worksheet->write("BC".$x,utf8_decode($PATRIMOINE));
		$worksheet->write("BD".$x,utf8_decode($C_PATRIMOINE_AUTRE));
		$worksheet->write("BE".$x,utf8_decode($C_COMT));
		$worksheet->write("BF".$x,utf8_decode($ALIMENTATION));
		$worksheet->write("BG".$x,utf8_decode($ALIMENTATION_AUTRE));
		$worksheet->write("BH".$x,utf8_decode($CONTEXT));
		$worksheet->write("BI".$x,utf8_decode($DECHETS));
		$worksheet->write("BJ".$x,utf8_decode($TAXON));
		$worksheet->write("BK".$x,utf8_decode($NOM_VERNACULAIRE));
		$worksheet->write("BL".$x,utf8_decode($TAXON_VEG));
		$worksheet->write("BM".$x,utf8_decode($NOM_VERNACULAIRE_VEG));
		$worksheet->write("BN".$x,utf8_decode($POURCENTAGE));
		$worksheet->write("BO".$x,utf8_decode($FAUNE));
		$worksheet->write("BP".$x,utf8_decode($LIAISON));
		$worksheet->write("BQ".$x,utf8_decode($LIAISON_AUTRE));
		$worksheet->write("BR".$x,utf8_decode($TRAVAUX));
		$worksheet->write("BS".$x,utf8_decode($TRAVAUX_AUTRE));
		$worksheet->write("BT".$x,utf8_decode($USAGE));
		...
		if($role_structure['ROLE'] == "administrateur"){
			...
			if($count_contour == 1){
				...
				$req_filtre_obsv = pg_query($bdd, 'SELECT  observation."L_ID",  observation."O_DATE", observation."O_REFE", observation."O_NLAT", observation."O_REID", observation."O_NVER", 
												   observation."O_REF2", observation."O_NLA2", observation."O_REI2", observation."O_NBRE", observation."O_NBRT", o_sacq."SACQ", 
												   observation."O_COMT", observateur."OBS_NOM_PRENOM", structure."STRUCTURE", o_styp."STYP"
											FROM saisie_observation.observation, menu_deroulant.o_sacq, saisie_observation.observateur, saisie_observation.structure, menu_deroulant.o_styp, saisie_observation.localisation
											WHERE observation."O_SACQ" = o_sacq."ID"
											AND saisie_observation.localisation."L_ID" = saisie_observation.observation."L_ID"
											AND observation."O_OBSV" = saisie_observation.observateur."ID"
											AND observation."O_STYP" = menu_deroulant.o_styp."ID"
		...
		$worksheet->write("BU".$x,$O_DATE);
		$worksheet->write("BV".$x,$O_REFE);
		$worksheet->write("BW".$x,utf8_decode($O_NLAT));
		$worksheet->write("BX".$x,$O_REID);
		$worksheet->write("BY".$x,utf8_decode($O_NVER));
		$worksheet->write("BZ".$x,$O_REF2);
		$worksheet->write("CA".$x,$O_NLA2);
		$worksheet->write("CB".$x,$O_REI2);
		$worksheet->write("CC".$x,$O_NBRE);
		$worksheet->write("CD".$x,utf8_decode($O_NBRT));
		$worksheet->write("CE".$x,utf8_decode($SACQ));
		$worksheet->write("CF".$x,utf8_decode($O_COMT));
		$worksheet->write("CG".$x,utf8_decode($OBS_NOM_PRENOM_OBSERVATION));
		$worksheet->write("CH".$x,utf8_decode($STRUCTURE_OBSERVATION));
		$worksheet->write("CI".$x,utf8_decode($STYP));
...
/////////////////////////////POUR GENERER LONGLET POUR LA LOCALISATION/////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Localisation'); //le param�tre ici est le nom de la feuille
$worksheet->set_column('A:P', 50); // le 30 repr�sente la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
										'merge' => 1)); // avec une marge
									 // 'fg_color' => 0x66));// coloration du fond des cellules
$headings = array('Identifiant PRAM', 'Identifiant de la mare structure', 'Nom usuel', 'Longitude', 'Latitude', 'X (RGG93)', 'Y (RGF93)', 'Num INSEE', 'Commune', 'Statut', 'Date de localisation', 'Observateur', 'Structure', 'Propriete', 'Commentaire', 'Validation du fond'); //d�finition du texte pour chaque cellules
...
$x=2;
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['L_ID']); // ici on va �crire une cellules bien d�finie
	$worksheet->write("B".$x,utf8_decode($resultat['L_IDSTRP']));
	$worksheet->write("C".$x,utf8_decode($resultat['L_NOM']));
	$worksheet->write("D".$x,$resultat['L_COOX']);
	$worksheet->write("E".$x,$resultat['L_COOY']);
	...
	$worksheet->write("M".$x,utf8_decode($resultat['STRUCTURE']));
	$worksheet->write("N".$x,utf8_decode($resultat['PROPR']));
	$worksheet->write("O".$x,utf8_decode($resultat['C_COMT']));
	$worksheet->write("P".$x,$resultat['L_VALID']);
	$x++;
	$ID_MARE = $resultat['L_ID'];
}
/////////////////////////////POUR GENERER LONGLET POUR LA CARACTERISATION/////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Caracterisation'); //le param�tre ici est le nom de la feuille
$worksheet->set_column('A:AO', 50); // le 30 repr�sente la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
										'merge' => 1)); // avec une marge
...
$headings = array('Identifiant de caracterisation', 'Identifiant de la mare', 'Date de caracterisation', 'Observateur', 'Structure', 'Type de mare', 'Vegetation',
					'Stade evolution', 'presence abreuvoir', 'Topographie', 'Precision topographie', 'Mare cloturee', 'Haie', 'Forme', 'Longeur', 'Largeur', 'Pronfondeur',
					'Nature du fond', 'nature du fond autre', 'Berges', 'Bourellet de curage', '% bourrelet de curage', 'Surpietinement', 'Regime hydrologique', 'Turbidite', 'Couleur specifique eau', 'Precision couleur',
					'Tampon','Exutoire', 'Recouvrement total', 'Recouvrement heliophyte', 'Recouvrement hydrophyte enracinees', 'Recouvrement hydrophyte non enracinees', 'Recouvrement algues', 'Eau libre', 'Recouvrement non vegetatlise', 
					'Embroussaillement', 'Ombrage', 'Petit patrimoine', 'Precision patrimoine', 'Commentaire'); //d�finition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int�gre notre texte et les le format de cellule.
// le premier param�tre correspond � la cellule o� l'on souhaite commencer � int�grer les diff�rent param�tre.
...
//SI count_contour = 1 alors on filtre les mares comprise dans le perimetre de la strucuture sinon on prend les mares qui posède l'identifiant en plus des filtres
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT saisie_observation.caracterisation."ID_CARAC" AS "ID_CARAC", "C_STRP", saisie_observation.caracterisation."L_ID" AS "L_ID", "C_DATE", "OBS_NOM_PRENOM", (SELECT "STRUCTURE" FROM saisie_observation.structure WHERE structure."S_ID"::text = "C_STRP"::text), "TYPE", "VEGET", "EVOLUTION", 
											"ABREUV", "TOPO", "C_TOPO", "C_TOPO_AUTRE", "CLOTURE", "HAIE", "FORM", "C_LONG", "C_LARG", "PROF", "NATFOND", "C_NATFOND", "C_NATFOND_AUTRE", "BERGES", "BOURRELET", 
											"C_BOURRELET_POURCENTAGE", "PIETINEMENT", "HYDROLOGIE", "TURBIDITE", "COULEUR", "C_COULEUR", "C_COULEUR_PRECISION", "TAMPON", "EXUTOIRE", "C_RECOU_TOTAL", "C_RECOU_HELOPHYTE", 
											"C_RECOU_HYDROPHYTE_E", "C_RECOU_HYDROPHYTE_NE", "C_RECOU_ALGUE", "C_RECOU_EAU_LIBRE", "C_RECOU_NON_VEGET", "EMBROUS", "OMBRAGE", "PATRIMOINE", "C_PATRIMOINE", "C_PATRIMOINE_AUTRE", saisie_observation.caracterisation."C_COMT" AS "C_COMT"
								FROM saisie_observation.caracterisation
								...
								LEFT JOIN menu_deroulant.c_type ON saisie_observation.caracterisation."C_TYPE" = menu_deroulant.c_type."ID"
								LEFT JOIN menu_deroulant.c_veget ON saisie_observation.caracterisation."C_VEGET" = menu_deroulant.c_veget."ID"
								WHERE saisie_observation.localisation."L_ANON" = '."'1'".'
								AND caracterisation."C_DATE" > '."'946681200'".' AND caracterisation."C_DATE" is not Null
								ORDER BY saisie_observation.caracterisation."ID_CARAC"');
}
elseif($role_structure['ROLE'] == "observateur"){
	...
	$req_filtre = pg_query($bdd, 'SELECT saisie_observation.caracterisation."ID_CARAC" AS "ID_CARAC", "C_STRP", saisie_observation.caracterisation."L_ID" AS "L_ID", "C_DATE", "OBS_NOM_PRENOM", (SELECT "STRUCTURE" FROM saisie_observation.structure WHERE structure."S_ID"::text = "C_STRP"::text), "TYPE", "VEGET", "EVOLUTION", 
								...
								LEFT JOIN menu_deroulant.c_type ON saisie_observation.caracterisation."C_TYPE" = menu_deroulant.c_type."ID"
								LEFT JOIN menu_deroulant.c_veget ON saisie_observation.caracterisation."C_VEGET" = menu_deroulant.c_veget."ID"
								WHERE (saisie_observation.structure."S_ID_SESSION" = '."'".$Identifiant_Session."'".' OR (saisie_observation.localisation."L_STRP"::text = structure."S_ID"::text AND structure."S_ID"::text = '."'".$id_structure_connectee."'".'))
...
$x=2;
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va �crire une cellules bien d�finie
	$worksheet->write("B".$x,$resultat['L_ID']); // ici on va �crire une cellules bien d�finie
	$worksheet->write("C".$x,date('d/m/Y', $resultat['C_DATE']));
	$worksheet->write("D".$x,utf8_decode($resultat['OBS_NOM_PRENOM']));
	$worksheet->write("E".$x,utf8_decode($resultat['STRUCTURE']));
	$worksheet->write("F".$x,utf8_decode($resultat['TYPE']));
	$worksheet->write("G".$x,utf8_decode($resultat['VEGET']));
	$worksheet->write("H".$x,utf8_decode($resultat['EVOLUTION']));
	$worksheet->write("I".$x,utf8_decode($resultat['ABREUV']));
	$worksheet->write("J".$x,utf8_decode($resultat['TOPO']));
	if($resultat['C_TOPO'] == "5"){
		$worksheet->write("K".$x,utf8_decode($resultat['C_TOPO_AUTRE']));
	}else{
		$worksheet->write("K".$x,'/');
	}
	$worksheet->write("L".$x,utf8_decode($resultat['CLOTURE']));
	$worksheet->write("M".$x,utf8_decode($resultat['HAIE']));
	$worksheet->write("N".$x,utf8_decode($resultat['FORM']));
	$worksheet->write("O".$x,utf8_decode($resultat['C_LONG']));
	$worksheet->write("P".$x,utf8_decode($resultat['C_LARG']));
	$worksheet->write("Q".$x,utf8_decode($resultat['PROF']));
	$worksheet->write("R".$x,utf8_decode($resultat['NATFOND']));
	if($resultat['C_NATFOND'] == "5"){
		$worksheet->write("S".$x,utf8_decode($resultat['C_NATFOND_AUTRE']));
	}else{
		$worksheet->write("S".$x,'/');
	}
	$worksheet->write("T".$x,utf8_decode($resultat['BERGES']));
	$worksheet->write("U".$x,utf8_decode($resultat['BOURRELET']));
	if($resultat['C_BOURRELET'] == "Oui"){
		$worksheet->write("V".$x,utf8_decode($resultat['C_BOURRELET_POURCENTAGE']));
	}else{
		$worksheet->write("V".$x,'/');
	}
	$worksheet->write("W".$x,utf8_decode($resultat['PIETINEMENT']));
	$worksheet->write("X".$x,utf8_decode($resultat['HYDROLOGIE']));
	$worksheet->write("Y".$x,utf8_decode($resultat['TURBIDITE']));
	$worksheet->write("Z".$x,utf8_decode($resultat['COULEUR']));
	if($resultat['C_COULEUR'] == "3"){
		$worksheet->write("AA".$x,utf8_decode($resultat['C_COULEUR_PRECISION']));
	}else{
		$worksheet->write("AA".$x,'/');
	}
	$worksheet->write("AB".$x,utf8_decode($resultat['TAMPON']));
	$worksheet->write("AC".$x,utf8_decode($resultat['EXUTOIRE']));
	
	$worksheet->write("AD".$x,utf8_decode($resultat['C_RECOU_TOTAL']));
	$worksheet->write("AE".$x,utf8_decode($resultat['C_RECOU_HELOPHYTE']));
	$worksheet->write("AF".$x,utf8_decode($resultat['C_RECOU_HYDROPHYTE_E']));
	$worksheet->write("AG".$x,utf8_decode($resultat['C_RECOU_HYDROPHYTE_NE']));
	$worksheet->write("AH".$x,utf8_decode($resultat['C_RECOU_ALGUE']));
	$worksheet->write("AI".$x,utf8_decode($resultat['C_RECOU_EAU_LIBRE']));
	$worksheet->write("AJ".$x,utf8_decode($resultat['C_RECOU_NON_VEGET']));
	$worksheet->write("AK".$x,utf8_decode($resultat['EMBROUS']));
	$worksheet->write("AL".$x,utf8_decode($resultat['OMBRAGE']));
	$worksheet->write("AM".$x,utf8_decode($resultat['PATRIMOINE']));
	if($resultat['C_PATRIMOINE'] == "6"){
		$worksheet->write("AN".$x,utf8_decode($resultat['C_PATRIMOINE_AUTRE']));
	}else{
		$worksheet->write("AN".$x,'/');
	}
	$worksheet->write("AO".$x,utf8_decode($resultat['C_COMT']));
	$x++;
	$ID_CARAC = $resultat['ID_CARAC'];
}

/////////////////////////////POUR GENERER LONGLET ALIMENTATIOn////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Alimentation'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:C', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
										'merge' => 1)); // avec une marge
...
$headings = array('Identifiant de caracterisation', 'Alimentation de la mare', 'Precision alimentation autre'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
...
//SI count_contour = 1 alors on filtre les mares comprise dans le perimetre de la strucuture sinon on prend les mares qui posède l'identifiant en plus des filtres
if($role_structure['ROLE'] == "administrateur"){
		$req_filtre =  pg_query($bdd, 'SELECT saisie_observation.caracterisation_alimentation."ID_CARAC", menu_deroulant.c_alimentation."ALIMENTATION", saisie_observation.caracterisation_alimentation."ALIMENTATION_AUTRE"
									FROM saisie_observation.caracterisation_alimentation, menu_deroulant.c_alimentation, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_alimentation."ALIMENTATION" = menu_deroulant.c_alimentation."ID"
									AND saisie_observation.caracterisation_alimentation."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
$x=2;
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['ALIMENTATION']));
	if($resultat['ALIMENTATION'] == "autre"){
		$worksheet->write("C".$x,utf8_decode($resultat['ALIMENTATION_AUTRE']));
	}else{
		$worksheet->write("C".$x,'/');
	}
...
/////////////////////////////POUR GENERER LONGLET POUR LE CONTEXT DE LA MARE////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Context_Mare'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:B', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
										'merge' => 1)); // avec une marge
...
$headings = array('Identifiant de caracterisation', 'Context de la mare'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  saisie_observation.caracterisation_context."ID_CARAC", menu_deroulant.c_context."CONTEXT"
									FROM saisie_observation.caracterisation_context, menu_deroulant.c_context, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_context."CONTEXT" = menu_deroulant.c_context."ID"
									AND saisie_observation.caracterisation_context."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['CONTEXT']));
	$x++;
}


/////////////////////////////POUR GENERER LONGLET DECHET ///////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Dechet'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:B', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de caracterisation', 'Dechets'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  saisie_observation.caracterisation_dechets."ID_CARAC", menu_deroulant.c_dechets."DECHETS"
									FROM saisie_observation.caracterisation_dechets, menu_deroulant.c_dechets, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_dechets."DECHETS" = menu_deroulant.c_dechets."ID"
									AND saisie_observation.caracterisation_dechets."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['DECHETS']));	
	$x++;
}


/////////////////////////////POUR GENERER LONGLET DES EAEEE///////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_EAEE'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:C', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de caracterisation', 'Taxon', 'Nom Vernaculaire'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  saisie_observation.caracterisation_eaee."ID_CARAC", menu_deroulant.c_eaee."TAXON", menu_deroulant.c_eaee."NOM_VERNACULAIRE"
									FROM saisie_observation.caracterisation_eaee, menu_deroulant.c_eaee, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_eaee."EAEE" = menu_deroulant.c_eaee."ID"
									AND saisie_observation.caracterisation_eaee."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,$resultat['TAXON']);	
	$worksheet->write("C".$x,utf8_decode($resultat['NOM_VERNACULAIRE']));	
	$x++;
}

/////////////////////////////POUR GENERER LONGLET DES EVEE///////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_EVEE'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:D', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
										'merge' => 1)); // avec une marge
...
$headings = array('Identifiant de caracterisation', 'Taxon', 'Nom Vernaculaire', 'Pourcentage'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  saisie_observation.caracterisation_evee."ID_CARAC", menu_deroulant.c_evee."TAXON", menu_deroulant.c_evee."NOM_VERNACULAIRE", menu_deroulant.c_evee_pourcent."POURCENTAGE"
									FROM saisie_observation.caracterisation_evee, menu_deroulant.c_evee, menu_deroulant.c_evee_pourcent, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_evee."EVEE" = menu_deroulant.c_evee."ID"
									AND saisie_observation.caracterisation_evee."EVEE_POURCENT" = menu_deroulant.c_evee_pourcent."ID"
									AND saisie_observation.caracterisation_evee."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
...
while($resultat = pg_fetch_array($req_filtre)){
	...
	$worksheet->write("C".$x,utf8_decode($resultat['NOM_VERNACULAIRE']));	
	$worksheet->write("D".$x,utf8_decode($resultat['POURCENTAGE']));	
	$x++;
}


/////////////////////////////POUR GENERER LONGLET Groupe Faunistique////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Groupe_Faunistique'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:B', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de caracterisation', 'Groupes faunistiques observee'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  saisie_observation.caracterisation_faune."ID_CARAC", menu_deroulant.c_faune."FAUNE"
									FROM saisie_observation.caracterisation_faune, menu_deroulant.c_faune, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_faune."FAUNE" = menu_deroulant.c_faune."ID"
									AND saisie_observation.caracterisation_faune."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['FAUNE']));	
	$x++;
}



/////////////////////////////POUR GENERER LONGLET POUR LA LIAISON////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Liaison_Mare'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:C', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de caracterisation', 'Liaison de la mare', 'Precision liaison autre'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  saisie_observation.caracterisation_liaison."ID_CARAC", menu_deroulant.c_liaison."LIAISON", saisie_observation.caracterisation_liaison."LIAISON_AUTRE"
									FROM saisie_observation.caracterisation_liaison, menu_deroulant.c_liaison, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE saisie_observation.caracterisation_liaison."LIAISON" = menu_deroulant.c_liaison."ID"
									AND saisie_observation.caracterisation_liaison."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['LIAISON']));
	if($resultat['LIAISON'] == "autre"){
		$worksheet->write("C".$x,utf8_decode($resultat['LIAISON_AUTRE']));
	}else{
		$worksheet->write("C".$x,'/');
	}
...
/////////////////////////////POUR GENERER LONGLET TRAVAUX/////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Travaux'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:C', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de caracterisation', 'Travaux envisagee', 'Precision travaux autres'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  caracterisation_travaux."ID_CARAC", c_travaux."TRAVAUX", caracterisation_travaux."TRAVAUX_AUTRE"
									FROM saisie_observation.caracterisation_travaux, menu_deroulant.c_travaux, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE caracterisation_travaux."TRAVAUX" = c_travaux."ID"
									AND saisie_observation.caracterisation_travaux."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['TRAVAUX']));
	if($resultat['TRAVAUX'] == "Autre"){
		$worksheet->write("C".$x,utf8_decode($resultat['TRAVAUX_AUTRE']));
	}else{
		$worksheet->write("C".$x,'/');
	}		
	$x++;
...
/////////////////////////////POUR GENERER LONGLET USAGE PRINCIPALE///////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Usage_Principale'); //le param鵲e ici est le nom de la feuille
$worksheet->set_column('A:B', 50); // le 30 repr괥nte la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de caracterisation', 'Usage de la mare'); //dꧩnition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int騲e notre texte et les le format de cellule.
// le premier param鵲e correspond ࡬a cellule o񠬧on souhaite commencer ࡩntꨲer les diff곥nt param鵲e.

//REQUETE SUR LA TABLE
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  caracterisation_usage."ID_CARAC", c_usage."USAGE"
									FROM saisie_observation.caracterisation_usage, menu_deroulant.c_usage, saisie_observation.caracterisation, saisie_observation.localisation, saisie_observation.structure
									WHERE caracterisation_usage."C_USAGE" = c_usage."ID"
									AND saisie_observation.caracterisation_usage."ID_CARAC" = saisie_observation.caracterisation."ID_CARAC"
									AND saisie_observation.caracterisation."L_ID" = saisie_observation.localisation."L_ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['ID_CARAC']); // ici on va ꤲire une cellules bien dꧩnie
	$worksheet->write("B".$x,utf8_decode($resultat['USAGE']));	
	$x++;
}


/////////////////////////////POUR GENERER LONGLET OBSERVATIONS/////////////////////////////////
$worksheet =& $workbook->addworksheet('PRAM_Observation_Faune_Flore'); //le param�tre ici est le nom de la feuille
$worksheet->set_column('A:P', 50); // le 30 repr�sente la largeur de chaque colonne
$heading  =& $workbook->addformat(array('bold' => 1, // on met le texte en gras
										'color' => 'black', // de couleur noire
									    'size' => 12, // de taille 12
...
$headings = array('Identifiant de la mare', 'Date de observation', 'Referentiel taxonomique faune', 'Taxon Taxref', 'ID Taxref', 'Nom vernaculaire', 'Referentiel taxonomique flore', 'Taxon CBNBL', 'ID CBNBL', 'Nombre', 'Denombrement', 'Methode acquisition', 'Commentaire', 'Observateur', 'Structure', 'Collecte'); //d�finition du texte pour chaque cellules
$worksheet->write_row('A1', $headings, $heading); //On int�gre notre texte et les le format de cellule.
// le premier param�tre correspond � la cellule o� l'on souhaite commencer � int�grer les diff�rent param�tre.
...
//SI count_contour = 1 alors on filtre les mares comprise dans le perimetre de la strucuture sinon on prend les mares qui posède l'identifiant en plus des filtres
if($role_structure['ROLE'] == "administrateur"){
	$req_filtre = pg_query($bdd, 'SELECT  "L_STRP", observation."L_ID",  observation."O_DATE", observation."O_REFE", observation."O_NLAT", observation."O_REID", observation."O_NVER", 
										   observation."O_REF2", observation."O_NLA2", observation."O_REI2", observation."O_NBRE", observation."O_NBRT", o_sacq."SACQ", 
										   observation."O_COMT", observateur."OBS_NOM_PRENOM", (SELECT "STRUCTURE" FROM saisie_observation.structure WHERE structure."S_ID"::text = "L_STRP"), o_styp."STYP"
									FROM saisie_observation.observation, menu_deroulant.o_sacq, saisie_observation.observateur, saisie_observation.structure, menu_deroulant.o_styp, saisie_observation.localisation
									WHERE saisie_observation.observation."L_ID" = saisie_observation.localisation."L_ID"
	...
	if($count_contour == 1){
		...
		$req_filtre = pg_query($bdd, 'SELECT  observation."L_ID",  observation."O_DATE", observation."O_REFE", observation."O_NLAT", observation."O_REID", observation."O_NVER", 
										   observation."O_REF2", observation."O_NLA2", observation."O_REI2", observation."O_NBRE", observation."O_NBRT", o_sacq."SACQ", 
										   observation."O_COMT", observateur."OBS_NOM_PRENOM", structure."STRUCTURE", o_styp."STYP"
									FROM saisie_observation.observation, menu_deroulant.o_sacq, saisie_observation.observateur, saisie_observation.structure, menu_deroulant.o_styp, saisie_observation.localisation
									WHERE observation."O_SACQ" = o_sacq."ID"
...
while($resultat = pg_fetch_array($req_filtre)){
	$worksheet->write("A".$x,$resultat['L_ID']); // ici on va �crire une cellules bien d�finie
	$worksheet->write("B".$x,date('d/m/Y',$resultat['O_DATE']));
	$worksheet->write("C".$x,$resultat['O_REFE']);
	$worksheet->write("D".$x,utf8_decode($resultat['O_NLAT']));
	$worksheet->write("E".$x,$resultat['O_REID']);
	$worksheet->write("F".$x,utf8_decode($resultat['O_NVER']));
	$worksheet->write("G".$x,$resultat['O_REF2']);
	$worksheet->write("H".$x,$resultat['O_NLA2']);
	$worksheet->write("I".$x,$resultat['O_REI2']);
	$worksheet->write("J".$x,$resultat['O_NBRE']);
	$worksheet->write("K".$x,utf8_decode($resultat['O_NBRT']));
	$worksheet->write("L".$x,utf8_decode($resultat['SACQ']));
	$worksheet->write("M".$x,utf8_decode($resultat['O_COMT']));
	$worksheet->write("N".$x,$resultat['OBS_NOM_PRENOM']);
	$worksheet->write("O".$x,utf8_decode($resultat['STRUCTURE']));
	$worksheet->write("P".$x,utf8_decode($resultat['STYP']));
	$x++;
}
