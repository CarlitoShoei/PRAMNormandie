...
elseif($role_structure['ROLE'] == "observateur"){
    ...
    if($count_contour == 1){
    $where_filtre_contour = 
    " , saisie_observation.structure where  c.\"L_ID\" = l.\"L_ID\" ".
    "AND ((st_intersects(structure.geom, st_transform(l.geom,2154)) ".
    "AND \"S_ID_SESSION\"='$Identifiant_Session') OR (l.\"L_STRP\"::text = structure.\"S_ID\"::text AND structure.\"S_ID\"::text = '$id_structure_connectee')) ";
    }else{
    $where_filtre_contour = 
    " , saisie_observation.structure s where  c.\"L_ID\" = l.\"L_ID\" ".
    "AND c.\"C_STRP\"::text = s.\"S_ID\"::text ".
...
elseif($role_structure['ROLE'] == "utilisateur"){
    ...
    " from ". 
    " (select loc.\"L_ID\", loc.geom, loc.\"L_STRP\" ". 
    " from saisie_observation.localisation loc ". 
    " where loc.\"L_ID\" is not null AND loc.\"L_ID\" <> '' ". 
    " group by 1,2,3 ". 
    " order by 1 ". 
    " ) as l, ". 
    " (". 
    ...
    " ) as c ".$where_filtre_contour. 
    " order by 1 ");


$ogr2ogr_query = $ogr2ogrPath." -f \"ESRI Shapefile\" \"".$shapefile_dir.$fname."\" PG:\""."host=$host port=$port dbname=$dbname user=$user password=$password"."\" "." -sql \"".$sql_query."\" -lco ENCODING=UTF-8 " ;
...
$zip = new ZipArchive();
$file_name_ = "shapefile_".date('H_i_s').$Identifiant_Session.".zip" ;
